...
class DynamoDB(SingletonBase):
    def __init__(self,dynamodb_instance):
        ...
        chats = Chats(dynamodb_instance)
        chats_logs = ChatLogs(dynamodb_instance)
        self.chats_table = chats.get_table(create_if_not_exists)
        self.chats_logs_table = chats_logs.get_table(create_if_not_exists)
        super().__init__(name)
