...
```bash
chatbot-app$ sam build --use-container
```
...
```bash
chatbot-app$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
chatbot-app$ sam local start-api
chatbot-app$ curl http://localhost:3000/
```
...
```bash
chatbot-app$ sam logs -n HelloWorldFunction --stack-name chatbot-app --tail
```
...
```bash
chatbot-app$ pip install -r tests/requirements.txt --user
# unit test
chatbot-app$ python -m pytest tests/unit -v
# integration test, requiring deploying the stack first.
# Create the env variable AWS_SAM_STACK_NAME with the name of the stack we are testing
chatbot-app$ AWS_SAM_STACK_NAME="chatbot-app" python -m pytest tests/integration -v
```
...
```bash
sam delete --stack-name "chatbot-app"
```
